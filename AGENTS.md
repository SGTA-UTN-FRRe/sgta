...
The current runtime does not yet include reporting, formal scholarship certification, or production operational workflows. The `src/auth/` and `src/db/` boundaries contain active secure-foundation implementations, while `src/features/` contains the implemented cycle/settings, tutor self-service and academic, scheduling/attendance, hour-accounting, and consultation slices plus reserved locations for later verticals.
