...
import { updateHourCategory } from "@/features/hours/hour-service";
import { updateHourCategoryInputSchema } from "@/features/hours/hour-validation";
import {
...
type HourCategoryRouteContext = {
  params: Promise<{ categoryId: string }>;
};

export async function PATCH(
  request: Request,
  context: HourCategoryRouteContext,
) {
  ...
  try {
    const category = await updateHourCategory(
      getDatabase(),
